...
uint8_t CTP_CFG_GT917S[] =  {
  0x46,0x20,0x03,0xE0,0x01,0x0A,0x35,0x04,0x00,0x69,
...
static int32_t GTP_Get_Info(void)
{
    ...
    ret = GTP_I2C_Read(GTP_ADDRESS, opr_buf, 8);
    if (ret < 0)
    {
        return FAIL;
    }
//    for(int i = 0; i < 8; i++)
//      GTP_ERROR("GTP i2c test:%02x",opr_buf[i]);    
...
static int8_t GTP_I2C_Test( void)
{
    uint8_t test[8] = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
    uint8_t retry = 0;
    int8_t ret = -1;
    ...
    while(retry++ < 5)
    {
        ret = GTP_I2C_Read(GTP_ADDRESS, test, 8);

        if (ret > 0)
        {
//            for(int i = 0; i < 8; i++)
//              GTP_ERROR("GTP i2c test:%02x",test[i]);
            return ret;
        }
